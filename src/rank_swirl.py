...
def write_circle(file, center, radius, stroke, fillcolor, strokecolor):
    file.write(
        '<circle class="node" r="{}" style="fill: rgb({},{},{}); stroke: rgb({},{},{}); stroke-width:{}" cx="{}" cy="{}"/>\n'.format(
            radius,
            fillcolor[0],
            fillcolor[1],
            fillcolor[2],
...
def write_arc(file, centerFrom, centerTo, anchor, width, color, opacity):
    file.write(
        '<path d="M{} {} Q {} {} {} {}" stroke-width="{}" stroke="rgb({},{},{})" stroke-opacity="{}" fill="none"/>\n'.format(
            centerFrom[0],
            centerFrom[1],
            anchor[0],
            anchor[1],
...
def write_text(file, xy, myText, myClass, myRotate, myTranslate):
    file.write(
        '<text x="{}" y="{}" class="{}" transform="translate({},{}) rotate({} {},{})">{}</text>\n'.format(
            xy[0],
            xy[1],
            myClass,
            myTranslate[0],
...
def write_line(file, myStart, myFinish, stroke, color, opacity):
    file.write(
        '<line x1="{}" y1="{}" x2="{}" y2="{}" stroke-width="{}" stroke="rgb({},{},{})" stroke-opacity="{}"/>\n'.format(
            myStart[0],
            myStart[1],
            myFinish[0],
            myFinish[1],
...
class RankSVG:
    ...
    def get_anchor(self, fr, to):
        ...
        anchor_x = (self.width / 2) + np.sign(to - fr) * np.abs(
            to - fr
        ) / self.aspectRatio
    ...
    def set_head(self):
        self.head = (
            '<svg width="{}" height="{}" id="{}" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">\n'.format(
                self.width, self.height, "v1"
            )
            + """
        <style>
    ...
    def write_center_line(self, f):
        center_line = svg_line(
            [self.width / 2, np.min(list(self.node2center.values())) - self.pad / 4],
            [self.width / 2, np.max(list(self.node2center.values())) + self.pad / 4],
            stroke=0.5,
    ...
    def label_axis(self, f):
        axis_text = svg_text(
            [self.width / 2 - 4, self.pad - 10], self.axis_label, "sans", -90, [0, 0]
        )
        f.write(axis_text)
    ...
    def write_under_edges(self, f):
        ...
        for edge in self.edges:
            if "width" in edge.keys():
                ...
                w = 2
            under_arc = svg_arc(
                self.node2center[edge["fr"]],
                self.node2center[edge["to"]],
                self.get_anchor(
                    self.node2center[edge["fr"]][1], self.node2center[edge["to"]][1]
                ),
                width=w + 2,
...
def nx2rankfigure(positions, nxG, filepath="viz.svg"):
    nodes = list(nxG.nodes())
    A = nx.to_numpy_array(nxG, nodelist=nxG.nodes(), weight="weight")
